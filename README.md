...
若使用者選擇區間內資料不足，將提醒使用者本程式因資料不足而無法回報分析結果，並等待一秒後自動跳轉回到主功能列。
...
>> 分析範圍：近三個月內（以90天計）
>> 行經期天數規律度分析結果：不規律！
>> 行經期長度分析結果：最近一次行經期長度長於普遍值！
>> 生理週期天數規律度分析結果：不規律！
>> 生理週期長度分析結果：最近一次生理週期長度介於普遍值
（行經期最近一次長度12天，生理週期最近一次長度35天）
>> 若出現驚嘆號表示分析結果異常，建議多加注意或諮詢婦產科。
===分析完畢，按enter回到主功能列===
>>
...
若檔案中三個月內資料不足，本程式將提醒使用者本程式因資料不足而無法回報分析結果，並等待一秒後自動跳轉回到主功能列。
...
若檔案中三個月內資料不足，本程式將提醒使用者本程式因資料不足而無法生成小提醒內容，並等待一秒後自動跳轉回到主功能列。
