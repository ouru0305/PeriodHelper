    ...
    while True:
        ...
        if len(period_data) == 0:
            ...
            if calc_res[1] != 'sta_sta':
                print('*生理週期、行經期天數不規律及資料紀錄不完整都會影響預測準確性。')
...
或按enter回到主功能列""")
    while True:
        ...
        if calc_res[0] == 'sta_sta':
            ...
            if calc_res[1] > 10:
                if calc_res[3] >35:
                    dur = '生理週期天數與行經期天數超過普遍值，建議諮詢婦產科。'
                elif calc_res[3] <14:
                    dur = '生理週期天數低於普遍值，行經期天數超過普遍值，建議諮詢婦產科。'
                else:
                    dur = '生理週期天數在普遍值內，行經期天數超過普遍值，建議諮詢婦產科。'
            elif calc_res[1] < 2:
                if calc_res[3] >35:
                    dur = '生理週期天數超過普遍值，行經期天數低於普遍值，建議諮詢婦產科。'
                elif calc_res[3] <21:
                    dur = '生理週期天數和行經期天數都低於普遍值，建議諮詢婦產科。'
                else:
                    dur = '生理週期天數低於普遍值，行經期天數在普遍值內，建議諮詢婦產科。'
            else:
                if calc_res[3] >35:
                    dur = '生理週期天數超過普遍值，行經期天數在普遍值內，建議諮詢婦產科。'
                elif calc_res[3] <21:
                    dur = '生理週期天數低於普遍值，行經期天數在普遍值內，建議諮詢婦產科。'
                else:
            ...
            if int(df.loc[index, 'period']) > 10:
                if 35 < (datetime.date.today() - (datetime.datetime.strptime(df.loc[index, 'end_date'], "%Y/%m/%d")).date()).days or int(df.loc[index, 'cycle']) >35:
                    dur = '最近一次生理週期天數及行經期天數皆超過普遍值，建議多加注意或諮詢婦產科。'
                elif int(df.loc[index, 'cycle']) <14:
                    dur = '最近一次生理週期天數低於普遍值，行經期天數超過普遍值，建議多加注意或諮詢婦產科。'
                else:
                    dur = '最近一次生理週期天數在普遍值內，行經期天數超過普遍值，建議多加注意或諮詢婦產科。'
            elif int(df.loc[index, 'period']) < 2:
                if 35 < (datetime.date.today() - (datetime.datetime.strptime(df.loc[index, 'end_date'], "%Y/%m/%d")).date()).days or int(df.loc[index, 'cycle']) >35:
                    dur = '最近一次生理週期天數超過普遍值，行經期天數低於普遍值，建議多加注意或諮詢婦產科。'
                elif int(df.loc[index, 'cycle']) <14:
                    dur = '最近一次生理週期天數和行經期天數皆低於普遍值，建議多加注意或諮詢婦產科。'
                else:
                    dur = '最近一次生理週期天數低於普遍值，行經期天數在普遍值內，建議多加注意或諮詢婦產科。'
            else:
                if 35 < (datetime.date.today() - (datetime.datetime.strptime(df.loc[index, 'end_date'], "%Y/%m/%d")).date()).days or int(df.loc[index, 'cycle']) >35:
                    dur = '最近一次生理週期天數超過普遍值，行經期天數在普遍值內，建議多加注意或諮詢婦產科。'
                elif int(df.loc[index, 'cycle']) <14:
                    dur = '最近一次生理週期天數低於普遍值，行經期天數在普遍值內，建議多加注意或諮詢婦產科。'
                else:
