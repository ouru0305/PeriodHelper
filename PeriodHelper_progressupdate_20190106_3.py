...
def calc(period, cycle):
    ...
    if period_std > 1.5:
        ...
        if cycle_std >5:
            ...
            det = 'sta_sta'
    return(det, period_avg, peravg_pri, cycle_avg, cycavg_pri)

def pred_prog(calc_res1, calc_res2, df, calc_res4):
    if calc_res1 == 'standard':
        index = len(df)-1
        n_per_start_date = datetime.datetime.strptime(df.loc[index, 'start_date'], "%Y/%m/%d").date() + timedelta(days = int(df.loc[index, 'period']))
...
def reminder():
    ...
    if len(period_data) == 0 or len(cycle_data) == 0:
        print('近三個月資料不足，無法生成小提醒！')
        print('即將自動回到功能列...')    
        time.sleep(1)
    else:
        ...
        elif 0 < (td - datetime.datetime.strptime(df.loc[index, 'end_date'], "%Y/%m/%d").date()).days < 7:
            print('行經期剛離開，現在是最佳進補時機喔。')
        else:
...
def pred():
    ...
    if len(period_data) == 0 or len(cycle_data) == 0:
        ...
        if calc_res[1] != 'sta_sta':
            print('*生理週期、行經期天數不規律及資料紀錄不足都會影響預測準確性。')
...
或按enter回到主功能列""")
    while True:
        ...
        if get_ran == "":
            ...
            print('輸入有誤喔！')
            continue
    if len(period_data) <= 1 or len(cycle_data) <= 1:
        print('資料不足，無法分析！')
        print('即將自動回到功能列...')
        time.sleep(1)
    else:
        ...
        if len(cycle_data) == 1:
            reg2 = '資料不足、無法分析'
            nam2 = '最近一次'
            lency = '最近一次長度' + str(df.loc[index, 'cycle']) + '天'
            dis_cy = int(df.loc[index, 'cycle'])
            if calc_res[0] == 'var_sta' or 'var_var':
                reg1 = '不規律！'
                nam1 = '最近一次'
                lenpe = '最近一次長度' + str(df.loc[index, 'period'])
                dis_pe = calc_res[1]
            elif calc_res[0] == 'sta_sta' or 'sta_var':
                reg1 = '規律'
                reg2 = '資料不足、無法分析'
                nam1 = '平均'
                nam2 = '最近一次' 
                lenpe = '平均長度' + str(calc_res[2])
                lency = '最近一次長度' + str(df.loc[index, 'cycle']) + '天'
                dis_pe = calc_res[1]
                dis_cy = int(df.loc[index, 'cycle'])
        else:                  
            if calc_res[0] == 'sta_sta':
                ...
                lency = '最近一次長度' + str(df.loc[index, 'cycle']) + '天'
                lenpe = '最近一次長度' + str(df.loc[index, 'period'])   
                dis_pe = int(df.loc[index, 'period'])
                dis_cy = int(df.loc[index, 'cycle'])
        ...
        if dis_cy > 35:
            ...
            dur2 = '介於普遍值'
        print('=======生理週期規律程度分析========')
        print(">> 分析範圍：{}\n>> 行經期天數規律度分析結果：{}\n>> 行經期長度分析結果：{}行經期長度{}".format(
            ran, reg1, nam1, dur1))
        print(">> 生理週期天數規律度分析結果：{}\n>> 生理週期長度分析結果：{}生理週期長度{}\n（行經期{}天，生理週期{}）".format(
            reg2, nam2, dur2, lenpe, lency))          
        print('>> 若出現驚嘆號表示分析結果異常，建議多加注意或諮詢婦產科。')
